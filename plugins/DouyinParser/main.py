...
class DouyinParser(PluginBase):
            ...
            async with aiohttp.ClientSession() as session:
                ...
                async with session.get(api_url, params=params, timeout=30, proxy=proxy) as response:  # 使用代理
                    ...
                    if data.get("code") == 200:
                        ...
                        # 获取真实视频链接
                        # if result.get('url'):
                            # result['url'] = await self._get_real_video_url(result['url'])
                            # result['url'] = await self._get_real_video_url(result['url'])
    ...
    async def _send_test_card(self, bot: WechatAPIClient, chat_id: str, sender: str):
        ...
        try:
            ...
            test_data = {
                ...
                'video': 'https://v3-reading.douyin.com/4acbff8028dd682665ea6c0c41917328/67bacab6/video/tos/cn/tos-cn-ve-15/ooADnRNBzB9ySEaiiAwKKAIVAAP5f3Bg7eExy0/?a=1967&ch=26&cr=3&dr=0&lr=all&cd=0%7C0%7C0%7C3&cv=1&br=2211&bt=2211&cs=0&ds=6&ft=h7TBU_JBRR0si0C4kDn2NcL4BL5sbL8tTb1U_4QvBTGSNv7TGW&mime_type=video_mp4&qs=0&rc=N2ZnZDVoZWhnaWk5Z2Y7NEBpM2x3eGw5cnRueDMzNGkzM0A0YDNeX14yXjUxLjI0YF9eYSMtaG1eMmRjMHBgLS1kLWFzcw%3D%3D&btag=80000e00030000&dy_q=1740290999&feature_id=f0150a16a324336cda5d6dd0b69ed299&l=202502231409591EEEA3E7BED19B2277A6',
                'title': '测试视频标题',
                'name': '测试作者',
    ...
    async def handle_douyin_links(self, bot: WechatAPIClient, message: dict):
        ...
        try:
            ...
            # 发送文字版消息
            # text_msg = (
            #     f"ð¬ 解析成功，微信内可直接观看（需ipv6）,浏览器打开可下载保存。\n"
            #     f"链接含有有效期，请尽快保存。\n"
            # )
            # if message['IsGroup']:
            #     text_msg = text_msg + "\n"
...
async def main():
    o = DouyinParser()
    content = "4.10 D@u.sE EUy:/ 06/03 一部被剧名耽误的科幻神剧，时间悖论究竟能否被打破？ # 一口气看完系列 # 科幻悬疑 # 美剧推荐 # 十二猴子  https://v.douyin.com/iPoYpqxU/ 复制此链接，打开Dou音搜索，直接观看视频！"
    match = o.url_pattern.search(content)
    if not match:
        return

    original_url = o._clean_url(match.group(0))
    logger.info(f"发现抖音链接: {original_url}")

    ret = await o._parse_douyin(original_url)
    print(ret)

# 4.10 D@u.sE EUy:/ 06/03 一部被剧名耽误的科幻神剧，时间悖论究竟能否被打破？ # 一口气看完系列 # 科幻悬疑 # 美剧推荐 # 十二猴子  https://v.douyin.com/iPoYpqxU/ 复制此链接，打开Dou音搜索，直接观看视频！
